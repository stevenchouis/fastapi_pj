...
這是 ECPay 官方規定的固定流程（所有語言的官方/社群 SDK 都是同一套），源自參考實作用 .NET 的 `HttpUtility.UrlEncode`（產生小寫 hex、且 `-_.!*()` 這幾個符號不跳脫、空白轉 `+`）。Python 的 `urllib.parse.quote_plus` 預設行為不完全一樣（`!*()` 會被跳脫成 `%21%2A%28%29`），所以要先整串 URL encode、轉小寫，再手動把這幾個字元對應的跳脫序列還原成原始符號，兩邊字串才會一致，MD5 雜湊出來的 `CheckMacValue` 才會跟 ECPay 那邊算出來的相符。這段邏輯已經寫自我一致性測試（產生 → 驗證通過、竄改 `CheckMacValue` → 驗證失敗），但**還沒有拿 ECPay 官方文件範例逐字比對過**，正式測試付款前建議照 ECPay 技術文件的範例參數再核對一次。
...
- **簽章演算法未經 ECPay 官方測試工具驗證**：目前只驗證了「產生→驗證自洽」，還沒有實際打過 ECPay 測試站的真實付款流程，也沒有拿官方文件的範例參數逐字核對過雜湊結果。正式測試付款前務必先跑一次端到端（用官方測試信用卡在 WebView 完成一筆付款，確認 `ReturnURL` 真的收得到通知、簽章驗證通過）。
