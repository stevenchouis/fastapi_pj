...
- **已知的相關技術債（一起 debug 時發現，尚未處理）：** `_publish_to_tokens` 目前完全沒讀 Expo 回傳的 push receipt，token 失效（`DeviceNotRegistered`）不會被清掉，`PushToken` 會持續累積 stale 資料。討論過用 `device_name` 當 upsert 鍵來順便清（見上），但因為裝置識別不夠精準而否決；建議之後改成讀 Expo receipt、收到 `DeviceNotRegistered` 才刪那筆 token，跟這次的 App 區分修復分開排工。
